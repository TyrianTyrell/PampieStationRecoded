...
/mob/living/carbon/proc/check_breath(datum/gas_mixture/breath)
	...
	if(breath.get_moles(/datum/gas/diapersmell))
		var/diapsmell_partialpressure = (breath.get_moles(/datum/gas/diapersmell)/breath.total_moles())*breath_pressure
		if(diapsmell_partialpressure > MINIMUM_MOLES_DELTA_TO_MOVE)

			if(prob(0.05 * diapsmell_partialpressure))
				var/datum/disease/advance/miasma_disease = new /datum/disease/advance/random(TRUE, 2,3)
				miasma_disease.name = "Unknown"
				ForceContractDisease(miasma_disease, TRUE, TRUE)

			//Miasma side effects
			switch(diapsmell_partialpressure)
				if(1 to 5)
					// At lower pp, give out a little warning
					SEND_SIGNAL(src, COMSIG_CLEAR_MOOD_EVENT, "sme")
					if(prob(5))
						to_chat(src, "<span class='notice'>There is an unpleasant smell in the air.</span>")
				if(5 to 20)
					//At somewhat higher pp, warning becomes more obvious
					if(prob(15))
						to_chat(src, "<span class='warning'>Something smells unmistakably like poop.</span>")
						SEND_SIGNAL(src, COMSIG_ADD_MOOD_EVENT, "sme", /datum/mood_event/disgust/bad_smell)
				if(15 to 30)
					//Small chance to vomit. By now, people have internals on anyway
					if(prob(5))
						to_chat(src, "<span class='warning'>The stench of poop is overwhemling!</span>")
						SEND_SIGNAL(src, COMSIG_ADD_MOOD_EVENT, "sme", /datum/mood_event/disgust/nauseating_stench)
				if(30 to INFINITY)
					//Higher chance to vomit. Let the horror start
					if(prob(25))
						to_chat(src, "<span class='warning'>The stench of poop is overwhemling!</span>")
