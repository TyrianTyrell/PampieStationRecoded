...
/mob/living/carbon/proc/help_shake_act(mob/living/carbon/M)
	...
	if(health >= 0 && !(HAS_TRAIT(src, TRAIT_FAKEDEATH)))
		...
		else if (M.zone_selected == BODY_ZONE_PRECISE_GROIN && (HAS_TRAIT(src, TRAIT_INCONTINENT) || HAS_TRAIT(src,TRAIT_FULLYINCONTINENT) || HAS_TRAIT(src,BABYBRAINED_TRAIT) || HAS_TRAIT(src,TRAIT_DIAPERUSE) || HAS_TRAIT(src,TRAIT_POTTYREBEL) || src.client.prefs.accident_types != "Opt Out") && M.get_active_held_item() == null) //Diaper checks!
