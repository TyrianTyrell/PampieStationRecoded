...
/datum/antagonist/obsessed/greet()
	...
	to_chat(owner, "<span class='boldannounce'>This role does NOT enable you to otherwise surpass what's deemed creepy behavior per the rules, nor violate other players' consent!</span>")
...
/datum/antagonist/obsessed/roundend_report()
	...
	if(trauma)
		...
		report += "<span class='redtext'>The [name] had no trauma attached to their antagonist ways! Either it bugged out or an admin improperly gave this good samaritan antag and it broke!</span>"
...
/datum/objective/assassinate/jealous/update_explanation_text()
	...
	if(target && target.current && old)
		explanation_text = "Teach [target.name], [old]'s coworker, a lesson."
	else
		explanation_text = "Free Objective"
...
/datum/antagonist/creep/proc/update_obsession_icons_added(var/mob/living/carbon/human/obsessed)
	var/datum/atom_hud/antag/creephud = GLOB.huds[ANTAG_HUD_OBSESSED]
	creephud.join_hud(obsessed)
	set_antag_hud(obsessed, "obsessed")

/datum/antagonist/creep/proc/update_obsession_icons_removed(var/mob/living/carbon/human/obsessed)
	var/datum/atom_hud/antag/creephud = GLOB.huds[ANTAG_HUD_OBSESSED]
