...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_tab)
		...
		if(APPEARANCE_TAB)
			...
			dat += "<b>Accident Types:</b><a style='display:block;width:100px' href='?_src_=prefs;preference=accident_types;task=input'>[accident_types]</a>"
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("breasts_fluid")
					var/new_fluid
					new_fluid = input(user, "Breast Fluid", "Character Preference") as null|anything in GLOB.genital_fluids_list
					if(new_fluid)
						features["breasts_fluid"] = new_fluid
...
/datum/preferences/proc/copy_to(mob/living/carbon/human/character, icon_updates = 1, roundstart_checks = TRUE, initial_spawn = FALSE)
	...
	if(additional_language && additional_language != "None")
		...
		if(language_entry)
			...
			character.grant_related_dialects(language_entry)
